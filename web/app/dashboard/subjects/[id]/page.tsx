          ...
          <div className="flex gap-0 divide-x divide-gray-100">
              ...
              <div className="py-3">
                ...
                <div className="mt-1">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                    subject.priority === 'critical' ? 'bg-red-100 text-red-700' :
                    subject.priority === 'high' ? 'bg-orange-100 text-orange-700' :
                    subject.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                    subject.priority === 'low' ? 'bg-green-100 text-green-700' :
                    'bg-slate-100 text-slate-600'
                  }`}>
                    {subject.priority ? subject.priority.charAt(0).toUpperCase() + subject.priority.slice(1) : '-'}
                  </span>
                </div>
              </div>
              <div className="pt-3">
            ...
            <div className="flex flex-1 flex-col divide-y divide-gray-100 pl-5">
              <div className="pb-3">
                ...
                <div className="mt-1">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                    subject.source_type === 'brand' ? 'bg-blue-100 text-blue-700' :
                    subject.source_type === 'dealer' ? 'bg-purple-100 text-purple-700' :
                    'bg-slate-100 text-slate-600'
                  }`}>
                    {subject.source_type ? subject.source_type.charAt(0).toUpperCase() + subject.source_type.slice(1) : '-'}
                  </span>
                </div>
              </div>
              <div className="py-3">
