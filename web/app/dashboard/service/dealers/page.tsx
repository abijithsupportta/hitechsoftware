...
export default function ServiceDealersPage() {
  ...
  const dueSummaryQuery = useQuery({
    queryKey: ['dealer-due-summary'],
    queryFn: async () => {
      const result = await supabase
        .from('subject_bills')
        .select('dealer_id,grand_total')
      ...
      for (const row of result.data ?? []) {
        const typed = row as { dealer_id: string; grand_total: number };
        const existing = map.get(typed.dealer_id) ?? { dueCount: 0, dueAmount: 0 };
        existing.dueCount += 1;
        existing.dueAmount += Number(typed.grand_total || 0);
        map.set(typed.dealer_id, existing);
      }
          ...
          <tbody className="divide-y divide-slate-100">
            {isLoading
              ? Array.from({ length: 5 }).map((_, index) => (
                  <tr key={`dealer-skeleton-${index}`} className="animate-pulse">
            ...
            {!isLoading && !error && data.length === 0 ? <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-slate-400">No dealers yet. Add one above.</td></tr> : null}
              ...
              <tr key={item.id} className="hover:bg-slate-50/50">
                <td className="px-4 py-3 text-sm font-medium text-slate-800">
                  {editingId === item.id ? (
                    ...
                    <Link href={ROUTES.DASHBOARD_SERVICE_DEALER_DETAIL(item.id)} className="text-blue-700 hover:text-blue-800 hover:underline">
                      {item.name}
                    </Link>
                  )}
                </td>
